...
/* phase:'both' 的構念在課前與課後各施一次（前測那一份是 ANCOVA 的共變數）。
   課前那一次，孩子還沒看過任何文章——所以這些題目一個字都不能寫「這篇文章」
   或「讀得這麼仔細」，那是在問一件還沒發生的事。
   eff 的第 1 題本來就寫「這樣長度的文章」，用的正是這個通則語氣；
   其餘四題沒跟上，改成同一個語氣（phase:'post' 的構念不受此限，
   它們只在課後施測，寫「這篇文章」是對的）。 */
const CONSTRUCTS = [
  ...
  {id:'mot_ex',  name:'外在動機',   dim:'學習動機', scale:SCALE6, phase:'both', cls:'sc2',
   src:'MSLQ 外在目標導向', items:[
    '我會認真讀這樣的文章，主要是因為會算成績',
    '我希望這次閱讀測驗的表現比班上多數同學好',
    '如果沒有人會看到結果，我大概就不會讀得那麼仔細']},
  ...
  {id:'eng_b',   name:'行為投入',   dim:'學習投入', scale:SCALE5, phase:'post', cls:'sc1',
   src:'Fredricks 三維·行為', items:[
    '整節課我都有跟著在讀',
    '我很少分心去做別的事',
    /* 原本是「該我回應的時候我都有回應」。三個 AI 組會讀成「回應夥伴」，
       對照組整節課沒有夥伴，只能讀成「回答題目」——同一個量尺點在四個條件
       指的是不同的行為，而行為投入正是要拿來做四條件比較的依變項。
       不能刪題（見 surveySections 的註解：正式施測要換成 Fredricks 原量表，
       刪題就不是那個構念了），所以改成把觸發條件講明白，四個條件同義。 */
    '需要我回答或寫下想法的時候，我都有做',
    '我把每一題都做完了']},
