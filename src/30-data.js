...
/* 題本（ITEMS 的 options 順序與 answer 索引）是種子契約的一部分：
...
/* ==========================================================================
   示範資料建構
   ========================================================================== */
/* 把示範班的白板複製給其餘班級。就地改寫傳入的 views／notes 陣列。
   remap 規則：
   ‧ 視圖與貼文取得新 id，viewId／links／buildOn／contains／refs.noteId 全部改指新 id
   ‧ 作者依「在班上的位次」對應到目標班的同一位次（老師仍是該班導師）
   ‧ 閱讀紀錄依目標班學生的參與傾向重擲
   ‧ 依 dropPlan 丟掉幾則葉節點，讓各班的參與量不同 */
function state_cloneBoards(views, notes, students, classes, srcStudents, dropPlan, rnd, now, DAY){
  const srcClassId = srcStudents.length ? srcStudents[0].classId : 'c-1';
  const srcViews = views.slice();
  const srcNotes = notes.slice();

  /* 葉節點＝沒有人延伸它、沒有被躍升收攏、也沒有被引用，而且不是共同問題錨點 */
  const referenced = {};
  srcNotes.forEach(function(n){
    if (n.buildOn) referenced[n.buildOn] = 1;
    (n.contains || []).forEach(function(c){ referenced[c] = 1; });
    (n.refs || []).forEach(function(r){ if (r.noteId) referenced[r.noteId] = 1; });
  });
  /* 共同問題錨點與躍升貼文一律不丟：那兩種是知識建構的結構標記，
     少一則會讓某些班在論述指標上看起來天生就少一種行為。 */
  const leaves = srcNotes.filter(function(n){
    return !referenced[n.id] && n.kind !== 'problem' && n.kind !== 'rise';
  }).map(function(n){ return n.id; });

  classes.forEach(function(k){
    if (k.id === srcClassId) return;
    const tgt = students.filter(function(s){ return s.classId === k.id; });
    if (!tgt.length) return;

    /* 這一班要丟掉哪幾則葉節點——固定種子，每次重建結果一致 */
    const drop = {};
    const nDrop = Math.min(dropPlan[k.id] || 0, leaves.length);
    const pool = leaves.slice();
    for (let d = 0; d < nDrop; d++){
      pool.splice(Math.floor(rnd() * pool.length), 1).forEach(function(id){ drop[id] = 1; });
    }

    const vMap = {}, nMap = {};
    srcViews.forEach(function(v, i){ vMap[v.id] = 'v-' + k.id + '-' + (i + 1); });
    srcNotes.forEach(function(n, i){ if (!drop[n.id]) nMap[n.id] = 'n-' + k.id + '-' + (i + 1); });

    function author(id){
      if (String(id).indexOf('u-s') !== 0) return k.teacherId;   // 老師 → 該班導師
      const idx = srcStudents.findIndex(function(s){ return s.id === id; });
      return idx >= 0 && tgt[idx] ? tgt[idx].id : k.teacherId;
    }

    srcViews.forEach(function(v){
      views.push(Object.assign({}, v, {
        id: vMap[v.id], classId: k.id,
        links: (v.links || []).map(function(l){ return vMap[l]; }).filter(Boolean)
      }));
    });
    ...
    srcNotes.forEach(function(n){
      ...
      const c = Object.assign({}, n, {
        id: nMap[n.id],
        viewId: vMap[n.viewId],
        authorIds: (n.authorIds || []).map(author),
        buildOn: n.buildOn ? (nMap[n.buildOn] || null) : null,
        contains: (n.contains || []).map(function(x){ return nMap[x]; }).filter(Boolean),
        refs: (n.refs || []).map(function(r){
          return Object.assign({}, r, {noteId: nMap[r.noteId]});
        }).filter(function(r){ return r.noteId; }),
        segs: (n.segs || []).map(function(g){ return Object.assign({}, g); }),
        annotations: (n.annotations || []).map(function(a){
          return Object.assign({}, a, {id: a.id + '-' + k.id, authorId: author(a.authorId)});
        }),
        reads: []
      });
      tgt.forEach(function(s){
        const p = 0.2 + 0.65 * s.engage;
        if (c.authorIds.indexOf(s.id) < 0 && rnd() < p) c.reads.push(s.id);
      });
      notes.push(c);
    });
  });
...
function buildSeedState(){
  ...
  const pre = {
    /* title 是學生端到處都印的名字（作業卡、作答頁抬頭、成績頁副標、
       知識建構空間的門檻卡），rlabel 只給教師端與研究端。
       原本兩份的 title 分別是「閱讀理解 前測」與
       「閱讀理解 評量即學習事件（後測）」——前後測是同一份 16 題，
       孩子在第一頁就被告知這是同一次測驗的第一次與第二次，
       而且後測那一份還把研究構念的名字（評量即學習事件）印在抬頭上。
       這兩件事各自都會改變他在後測的作答姿態（重測覺察與需求特徵），
       而後測 θ 正是 RQ1 的主要依變項；四個條件都印同一句，
       所以它不是組間差異，是把整條 Δθ 一起抬起來或壓下去的共同偏誤。
       學生端只看得到兩個中性的活動名稱，順序資訊不可避免也無害。 */
    id:'a-pre', title:'閱讀活動（一）', rlabel:'前測',
    desc:'先看看大家目前的讀法，答錯沒關係，等一下我們一起討論。',
    classIds: allClassIds, teacherId:'u-t1', itemIds: allIds, phase:'pre',
    createdAt: now - 12 * DAY, due: now - 9 * DAY
  };
  const post = {
    id:'a-post', title:'閱讀活動（二）', rlabel:'評量即學習事件（後測）',
  ...
  const v1 = {
    ...
    /* 白板是學生讀的。「前測第 8 題」與題本抬頭是同一個問題：
       前後測同一份 16 題，displayNo 也同源，所以「第 8 題」本身
       就足以指出是哪一題，多的那兩個字只提供「這是同一次測驗的第一次」
       這一個訊息。見 30-data.js 上面 a-pre／a-post 的 rlabel 註解。 */
    desc:'從第 8 題長出來的共同問題：這是巧合，還是作者早就安排好的？',
  ...
  mkNote({v:'v-1', t:'【全班共同問題】結尾是巧合，還是作者早就安排好的？', kind:'problem',
    ...
    segs:[{s:'s2', text:'第 8 題問「作者在前面埋了哪一個線索」，有不少同學選了跟結尾無關的細節。有趣的是，這些同學在比較難的第 7 題反而答對了——照他們的能力，這一題本來應該會。我們一起來弄清楚：什麼樣的細節才算「伏筆」？'}]});
  ...
  mkNote({v:'v-2', t:'【全班共同問題】答對了，但說得出理由嗎？', kind:'problem',
    ...
    segs:[{s:'s2', text:'第 9 題有不少同學選對了，但寫下的理由是「因為老人家很辛苦」——那是你的想法，不是文章的訊息。請大家想清楚：怎樣才算真的從文章裡讀出來的？'}]});
  ...
  /* --- 把示範白板複製到其餘三個班 ---
     白板依班級隔離之後，只種示範班會讓另外三個條件的知識建構空間天生是空的。
     知識建構參與度是依變項之一，空白板本身就成了條件間的介面差異，
     示範畫面也會讓人誤以為對照組沒有這個環節。

     但四塊一模一樣的白板同樣不可信——雙軌儀表板會呈現四個完全相同的
     KB 側寫。所以複製時保留結構、依班改變參與量：用同一顆固定種子
     丟掉幾則「葉節點」貼文（沒有人延伸、沒有被收攏、也沒有被引用的那些），
     再各自重擲閱讀紀錄。這樣四個班的貼文數、延伸數與閱讀廣度都不同，
     而想法串的結構完整。 */
  const CLONE_DROP = {'c-2': 2, 'c-3': 4, 'c-4': 3};   // 各班要丟掉幾則葉節點
  state_cloneBoards(views, notes, students, classes, kbStudents, CLONE_DROP, rnd, now, DAY);
  ...
  const st = {
    ...
    settings: {
      ...
      /* 空字串：代碼在清場（goLiveCommit）時才產生，一台裝置一組。
         原本寫死 '1234'，而且沒有任何 UI 改得動－－四個班 96 台平板
         同一組碼，而 1234 是十歲孩子第一個會試的四位數。 */
      teacherCode: '',
      /* 解鎖嘗試的紀錄（次數與下一次可以試的時間），用來做退避。 */
      unlockTries: 0, unlockLockedUntil: 0,
