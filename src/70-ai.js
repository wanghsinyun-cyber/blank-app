...
function builtinItemStrategy(pi){
  ...
  const acts = (pi.misCode && STRATEGY[pi.misCode]) || [
    '請兩位落在迷思象限的同學上台講自己的想法，不要先糾正。',
    '給一個相同結構、數字更小的題目，讓他們重走一次流程。',
    '要求把答案代回原式檢驗，並把檢驗過程寫下來。'];
  acts.forEach(function(a){ L.push('- ' + a); });
  L.push('');
  L.push('#### 錯誤類型判斷');
  const type = !m ? '策略錯' : (['M1','M3','M7'].indexOf(m.id) >= 0 ? '概念錯' : (['M2','M4'].indexOf(m.id) >= 0 ? '程序錯' : '程序錯'));
  L.push('**' + type + '**：' + (type === '概念錯'
    ? '學生對符號或定義本身的理解就有偏差，補練習沒有用，必須回到定義重新協商。'
    : type === '程序錯'
      ? '學生知道概念，但漏掉了一個必要步驟，適合用固定的檢核流程來矯正。'
      : '學生缺的是「什麼時候該用哪一招」的判斷，適合用題目分類活動處理。'));
  L.push('');
  L.push('#### 建議的共構切入點');
  L.push('把這一題貼進知識建構空間時，不要問「正確答案是什麼」，改問「' +
    (m ? m.name.replace(/^./, function(c){ return c; }) : '這個做法') + '在什麼情況下會出錯？請舉一個能檢驗的例子」。');
  return L.join('\n');
}
...
function builtinRubric(item){
  ...
  L.push('| --- | --- | --- |');
  if (item.id === 'C01'){
    L.push('| 策略／概念 | 2 | 選用因式分解、配方法或公式解其中一種，且該方法對此題可行；有寫出所選方法的名稱或關鍵步驟。 |');
    L.push('| 執行／計算 | 3 | 移項成 ＝0；正確求得 x=3 與 x=1/2 兩個根；過程無運算錯誤。只求得一根給 1 分。 |');
    L.push('| 表達／結論 | 1 | 有把根代回原式檢驗，並明確寫出結論。 |');
    L.push('');
    L.push('#### 扣分要點');
    L.push('- **只寫一個根**：扣 2 分。二次方程式在此題有兩相異實根，漏根屬概念性缺漏。');
    L.push('- **未移項就分解**（如由 2x²−7x=−3 直接拆）：扣 2 分，並在評語中指出乘積為零性質的使用條件。');
    L.push('- **沒有檢驗**：扣 1 分。即使答案正確，也要提醒代回原式是解方程式的必要習慣。');
  } else {
    L.push('| 策略／概念 | 2 | 能指出小明的說法不完整，並說明「平方等於 25 的數」不只一個。 |');
    L.push('| 執行／計算 | 2 | 正確寫出 x=5 或 x=−5，並用 (−5)²=25 佐證。 |');
    L.push('| 表達／結論 | 2 | 舉出一個具體例子（情境或數例）支持說明；若能同時指出「√25=5」與「x²=25 的解」是不同問題，給滿分。 |');
    L.push('');
    L.push('#### 扣分要點');
    L.push('- **只回答「對」或「錯」而無理由**：僅給 1 分。本題評的是論證，不是判斷。');
    L.push('- **把 √25 寫成 ±5**：扣 1 分，並在評語中區分算術平方根與方程式的解。');
    L.push('- **沒有舉例**：扣 2 分。舉例是本題明確要求的表現。');
  }
  L.push('');
...
const SIMILAR_BANK = {
  Q01:[{stem:'若 x² = 81，則 x 的所有可能值為何？', options:['x = 9','x = 9 或 x = −9','x = 9 或 x = 0','x = 40.5'], answer:1, hint:'問的是「所有平方後等於 81 的數」，正負都要寫。'},
       {stem:'若 x² = 5，則 x 的所有可能值為何？', options:['x = √5','x = ±√5','x = 2.5','沒有解'], answer:1, hint:'不是完全平方數也一樣有兩個根，寫成 ±√5。'},
       {stem:'方程式 (x + 2)² = 36 的解為何？', options:['x = 4','x = 4 或 x = −8','x = 6 或 x = −6','x = 34'], answer:1, hint:'先開方得 x+2 = ±6，再分別解出兩個 x。'}],
  Q03:[{stem:'解方程式 x(x + 2) = 8 時，下列哪一個第一步是正確的？', options:['x = 8 或 x + 2 = 8','先展開並移項成 x² + 2x − 8 = 0','把 8 拆成 2×4','兩邊同除以 x'], answer:1, hint:'乘積為零性質只有在一邊為 0 時能用。'},
       {stem:'下列哪一個方程式可以直接使用「乘積為零」性質？', options:['(x − 1)(x + 4) = 0','(x − 1)(x + 4) = 6','x(x + 3) = 10','2(x − 5) = 8'], answer:0, hint:'看等號右邊是不是 0。'},
       {stem:'若 (2x − 6)(x + 1) = 0，則 x 的值為何？', options:['3 或 −1','−3 或 1','6 或 −1','3 或 1'], answer:0, hint:'兩個括號分別令為 0 後各自解出。'}],
  Q07:[{stem:'計算 √25 + √4 之值為何？', options:['7','√29','29','5.4'], answer:0, hint:'先各自開方再相加，根號不能逐項合併。'},
       {stem:'下列敘述何者正確？', options:['√9 + √16 = √25','√9 × √16 = √144','√9 + √16 = 7 且 √25 = 5，兩者不相等','√25 = ±5'], answer:2, hint:'加法不能進根號，但乘法可以。'},
       {stem:'計算 √36 − √9 之值為何？', options:['3','√27','√45','27'], answer:0, hint:'6 − 3 = 3；減法一樣要先各自開方。'}],
  Q10:[{stem:'直角三角形的斜邊長 25，一股長 7，另一股長為何？', options:['24','√674','18','32'], answer:0, hint:'25² − 7² = 625 − 49 = 576，開方得 24。'},
       {stem:'一個直角三角形兩邊長為 9 與 15，且 15 為斜邊，則第三邊長為何？', options:['12','√306','6','24'], answer:0, hint:'先確認哪一邊是斜邊，再用相減。'},
       {stem:'梯子長 13 公尺靠牆，底端離牆 5 公尺，梯頂離地多高？', options:['12 公尺','√194 公尺','8 公尺','18 公尺'], answer:0, hint:'梯子本身是斜邊。'}],
  Q12:[{stem:'將 3x² + 10x + 3 因式分解，結果為何？', options:['(3x + 1)(x + 3)','(3x + 3)(x + 1)','(x + 1)(x + 3)','(3x + 10)(x + 3)'], answer:0, hint:'交叉相乘後檢驗一次項係數是否為 10。'},
       {stem:'將 2x² − 5x − 3 因式分解，結果為何？', options:['(2x + 1)(x − 3)','(2x − 1)(x + 3)','(2x − 3)(x + 1)','(x − 1)(2x + 3)'], answer:0, hint:'常數項為負，兩括號的常數一正一負。'},
       {stem:'下列哪一組是 6x² + 7x + 2 的因式分解？', options:['(2x + 1)(3x + 2)','(6x + 1)(x + 2)','(3x + 1)(2x + 2)','(6x + 2)(x + 1)'], answer:0, hint:'首項與常數項都要拆，再檢驗中間項 4x + 3x = 7x。'}]
};

function builtinSimilar(item){
  if (SIMILAR_BANK[item.id]) return SIMILAR_BANK[item.id];
  // 沒有預備題組時，用同單元、同難度的題目做為練習
  const alt = ITEMS.filter(function(i){
    return i.id !== item.id && i.unit === item.unit && i.type === 'mc';
  }).slice(0, 3).map(function(i){
    return {stem:i.stem, options:i.options, answer:i.answer, hint:i.note};
  });
  return alt.length ? alt : [{stem:'（內建題庫尚未為這一題準備相似題。切換到外部語言模型即可即時生成。）',
    options:['—','—','—','—'], answer:0, hint:'到「系統設定」填入 OpenAI 相容 API key。'}];
}
