...
const CONDITIONS = [
  ...
  {id:'peer',    name:'同學小森', en:'AI-as-Peer',   cls:'sc5', mark:'■',
   ...
   /* 原本是「我先說我讀到哪裡，你再說你的」——但夥伴永遠不會主動開口
      ...
      改文案而不是改劑量：不承諾先後。 */
   frame:'我也在讀這一篇。我們各自說說看自己怎麼讀，再對一下。',
   mech:'分享觀點、共構理解',
   note:'處理負擔分散於人機之間。'},
...
const PROMPT_ROLE = {
  ...
  peer:  '【社會框架】你是跟學生同時在讀這一篇的同學。先簡短說一句你自己讀到哪裡或卡在哪裡' +
         '（不可以是答案，也不可以說第幾段或第幾句），再把同一個問題拋回去問他，語氣是對等的討論。'
};
...
function leakGuard(text, item){
  ...
  /* 句與段都要擋，而且要認中文數字——原本只有一條阿拉伯數字的句次規則，
     於是「第二段」「第二句」都通過。 */
  if (item && item.answerSent != null && posRe(item.answerSent + 1, '句').test(t)){
    hits.push('指名答案所在句次');
  }
  if (item && item.answerPara != null && posRe(item.answerPara + 1, '段').test(t)){
    hits.push('指名答案所在段落');
  }
  if (hits.length){
